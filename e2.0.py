...
def init_database():
    """初始化SQLite数据库表结构"""
    # 确保metadata目录存在
    os.makedirs(os.path.dirname("metadata/image_index.db"), exist_ok=True)
    
    conn = sqlite3.connect("metadata/image_index.db")
    cursor = conn.cursor()
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS images (
            ...
            filepath TEXT UNIQUE,   -- 存储完整的Cloudinary public_id
            prompt_text TEXT,
            type TEXT,
            style TEXT,
            model_name TEXT,
...
def load_images_from_cloudinary_to_db(force_refresh=False):
    ...
    try:
            ...
            while True:
                try:
                    ...
                    for res in batch_resources:
                        # 获取完整的public_id
                        filename = res["public_id"]  # 注意：使用folders参数时，这里只返回文件名
                        ...
                        if len(parts) >= 5:
                            # 寻找模型名在文件名中的位置
                            model_in_filename = None
                            possible_models = ['dalle3', 'dreamshaper', 'sd15', 'sdxl_turbo', 'sdxl']
                            
                            for i, part in enumerate(parts):
                                if part in possible_models:
                                    model_in_filename = part
                                    # 尝试获取图片编号
                            ...
                            if not model_in_filename:
                                # 如果没找到模型名，使用简化逻辑
                                prompt_id = '_'.join(parts[:-2]) if len(parts) >= 3 else filename
                                try:
                                    image_number = int(parts[-2]) if parts[-2].isdigit() else 1
                        ...
                        if total_loaded % 100 == 0:
                            conn.commit()
                            status_text.text(f"✅ 已加载 {total_loaded} 张图片...")
        ...
        st.success(f"🎉 成功从Cloudinary加载 {final_count} 张图片到数据库！")
...
def main_rating_page():
    ...
    with st.sidebar:
        ...
        if not os.path.exists("metadata/image_index.db"):
            st.warning("⚠️ 数据库未初始化")
            if st.button("初始化数据库并加载数据"):
                with st.spinner("初始化数据库并从Cloudinary加载图片资源..."):
                    init_database()
                    count = load_images_from_cloudinary_to_db()
                    if count > 0:
                        st.success(f"✅ 成功加载 {count} 张图片")
            ...
            if img_count == 0:
                st.warning("⚠️ 数据库为空")
                if st.button("从Cloudinary加载数据"):
                    with st.spinner("从Cloudinary加载图片资源..."):
                        count = load_images_from_cloudinary_to_db()
                        if count > 0:
                            st.success(f"✅ 成功加载 {count} 张图片")
                            st.session_state.data_loaded = True
                ...
                st.success(f"✅ 数据库中有 {img_count} 张图片")
                
                if st.button("强制重新加载数据"):
                    with st.spinner("重新加载所有数据..."):
                        count = load_images_from_cloudinary_to_db(force_refresh=True)
                        if count > 0:
                            st.success(f"✅ 重新加载 {count} 张图片")
                            st.session_state.data_loaded = True
                            st.rerun()
    ...
    if img_count == 0:
        st.warning("⚠️ 数据库中没有图片数据，请在侧边栏加载数据")
        return
    ...
    for idx, row in page_images.iterrows():
        # 显示图片信息（不使用expander以避免嵌套问题）
        st.markdown(f"### 🖼️ {row['prompt_id']} | {row['model_name']} | 图片{row['image_number']}")
        
        col_img, col_form = st.columns([1, 2])

        # 左侧：图片展示
        with col_img:
            # 直接使用数据库中的filepath作为public_id
            ...
            try:
                ...
                st.error("❌ 无法加载图片")
                st.write(f"**Public ID:** {row['filepath']}")
            
            # 显示基本信息（不使用expander）
            st.markdown(f"""
            **Prompt:** {row['prompt_text'][:100]}{'...' if len(row['prompt_text'])>100 else ''}  
            **类型:** {row['type']} | **风格:** {row['style']}  
            **模型:** {row['model_name']} ({row['quality_tier']})  
            **上传时间:** {row['generation_time'][:19]}
            """)
