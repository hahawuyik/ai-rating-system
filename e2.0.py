...
def main():
    ...
    with col_form:
        ...
        with st.form(key=f"form_{row['id']}"):
            ...
            c1, c2 = st.columns(2)
            with c1:
                perspective = st.slider("透视准确性 (Perspective)", 1, 5, existing.get('perspective_check', 3), 
                                      help="透视是否扭曲？是否符合特定的游戏视角（如ISO/顶视图）？")
                asset_clean = st.slider("资产干净度 (Cleanliness)", 1, 5, existing.get('asset_cleanliness', 3), 
                                      help="边缘是否清晰？背景是否容易去除（Matting）？有无伪影？")
            with c2:
                struct_logic = st.slider("结构逻辑性 (Structure)", 1, 5, existing.get('structural_logic', 3), 
